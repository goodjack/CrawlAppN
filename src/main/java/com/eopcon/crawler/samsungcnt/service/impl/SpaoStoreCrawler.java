 ...
 * 스파오 사이트 크롤러
 */
public class SpaoStoreCrawler extends OnlineStoreCrawler implements InitializingBean {

	private final String PAGE_ENCODING = "utf-8";

	public final static String BASE_URL = "http://spao.elandmall.com";
	public final static String BASE_HOST = "spao.elandmall.com";
	public final static String CATEGORY_PAGE_URL = "http://spao.elandmall.com/dispctg/searchGnbAllCategoryList.action";
	public final static String CATEGORY_DETAIL_PAGE_URL = "http://spao.elandmall.com/dispctg/initDispCtg.action?disp_ctg_no=";
	public final static String PRODUCT_STOCK_INFO_URL = "http://spao.elandmall.com/goods/searchGoodsItemList.action";
	public final static String PRODUCT_COMMENT_INFO_URL = "http://spao.elandmall.com/goods/searchGoodsEvalBody.action";
	public final static String PRODUCT_BEST_ITEM_INFO_URL = "http://spao.elandmall.com/shop/initBestBrandMall.action";

	private SpaoStoreParser parser;

	public SpaoStoreCrawler(ServiceConfig config, OnlineStoreConst constant) {
	...
	public void afterPropertiesSet() throws Exception {
		...
		this.parser = (SpaoStoreParser) super.parser;
	}
	...
	public List<Category> getCategories() throws Exception {
		...
		if (responseCode == HttpStatus.SC_OK) {
			...
			logger.debug("CATEGORY_PAGE_URL: "+  content);
	...
	public void fillOutProductDetail(String productURL, ProductDetail productDetail) throws Exception {
		...
		if (responseCode == HttpStatus.SC_OK) {
			...
			fillOutStockInfo(SpaoStoreParser.FLAG_PARSING_COLOR, productDetail, param);
	...
	public void fillOutStockInfo(int flag, ProductDetail productDetail, Map<String, String> param) throws Exception {
		...
		switch (flag) {
		case SpaoStoreParser.FLAG_PARSING_SIZE:
		...
		if (responseCode == HttpStatus.SC_OK) {
			...
			switch (flag) {
			case SpaoStoreParser.FLAG_PARSING_COLOR:
				String[] colors = param.get(OnlineStoreConst.KEY_COLLECT_COLORS).split("; ");
				for (String color : colors) {
					param.put("opt_val_nm1", color);
					fillOutStockInfo(SpaoStoreParser.FLAG_PARSING_SIZE, productDetail, param);
				}
				break;
			}
		} else {
	...
	/**
	 * SKU 번호/명/색상 정보를 가져온다. ※ SPAO의 경우 괄호안에 색상코드를 포함한다. ex) (10)WHITE
	 */
	@Override
